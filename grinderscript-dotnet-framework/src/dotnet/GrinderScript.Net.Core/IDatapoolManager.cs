...
// --------------------------------------------------------------------------------------------------------------------
// <copyright file="IDatapoolManager.cs" company="http://GrinderScript.net">
//
...
namespace GrinderScript.Net.Core
{
    public interface IDatapoolManager
    {
        ...
        void BuildDatapool<T>(IDatapoolMetatdata<T> metadata) where T : class;

        bool ContainsDatapool<T>();

        bool ContainsDatapool(string name);
    }
}